package com.dendryt.software.client.programmer;
...
import com.dendryt.software.client.designer.DesignerController;
...
public class ProgrammerInterface implements IUserInterface {
	private RootPanel rootPanel;
	private ProgrammerController designerController;
	...
	public ProgrammerInterface(String userName) {
...
//		});
		...
		designerController = new ProgrammerController(this);
		rootPanel = RootPanel.get();
	}

	public void mainInterface() {
		Window.setTitle("Dendryt - Interfejs programisty");
		...
		//stackPanel.add(addNotification(), "Dodawanie zgloszen");
	...
	HorizontalPanel notificationsRevisionInterface() {
		...
		final Button raporty = new Button("Raporty", new ClickListener() {
			...
			public void onClick(Widget sender) {
				...
				stackPanel.remove(0);
				stackPanel.add(reportsRevisionInterface(), "Przeglad raportow (i zgloszen)");
				stackPanel.showStack(0);
			}
		});
	...
	public void setSelfNotificationsRevisionData(List<Zgloszenie> selfNotificationsList) {
		...
		for(int i = 0; i < selfNotificationsList.size(); i++) {
			System.err.println(selfNotificationsList.get(i).getProdukt());
	...
	Grid addingReport() {
		...
		final Button dalej = new Button("Do testera", new ClickListener() {
		...
		final Button wczesniej = new Button("Do projektanta", new ClickListener() {
	...
	HorizontalPanel selfReportsRevisionInterface() {
		...
		if(((selfReportsList.get(selfReportsList.size() - 1).userType == IFunkcje.PROJEKTANT) && selfNotificationsList.get(selfNotificationsListWidget.getSelectedIndex()).doPrzodu()) ||
				((selfReportsList.get(selfReportsList.size() - 1).userType == IFunkcje.TESTER) && !selfNotificationsList.get(selfNotificationsListWidget.getSelectedIndex()).doPrzodu()))
	...
	Zgloszenie dodaneZgloszenie;
	
	public void notificationAdded() {
		if(selfNotificationsList != null)
			selfNotificationsList.add(dodaneZgloszenie);
		
		if(selfNotificationsListWidget != null)
			selfNotificationsListWidget.addItem(dodaneZgloszenie.getProdukt());
	}
	
	
	Grid addNotification() {
		Grid addingReportGrid = new Grid(9, 3);
		
		addingReportGrid.setWidget(0, 0, new Label("Produkt:"));
		addingReportGrid.setWidget(1, 0, new Label("Imie zglaszajacego:"));
		addingReportGrid.setWidget(2, 0, new Label("Nazwisko zglaszajacego:"));
		addingReportGrid.setWidget(3, 0, new Label("Telefon zglaszajacego:"));
		addingReportGrid.setWidget(4, 0, new Label("Waga zglaszajacego:"));
		addingReportGrid.setWidget(5, 0, new Label("Pracownik:"));
		//addingReportGrid.setWidget(6, 0, new Label("Typ pracownika:"));
		//addingReportGrid.setWidget(3, 0, new Label("Data:"));
		addingReportGrid.setWidget(7, 0, new Label("Komentarz:"));
		
		//final TextBox produkt = new TextBox();
		
		
		final TextBox imie = new TextBox();
		addingReportGrid.setWidget(1, 1, imie);
		
		final TextBox nazwisko = new TextBox();
		addingReportGrid.setWidget(2, 1, nazwisko);
		
		final TextBox telefon = new TextBox();
		addingReportGrid.setWidget(3, 1, telefon);
		
		final TextBox waga = new TextBox();
		addingReportGrid.setWidget(4, 1, waga);
		
		final TextBox pracownik = new TextBox();
		pracownik.setEnabled(false);
		addingReportGrid.setWidget(5, 1, pracownik);
		
		/*final TextBox typ = new TextBox();
		typ.setEnabled(false);
		addingReportGrid.setWidget(6, 1, typ);
		...
		final Button pobierz = new Button("Pobierz produkty", new ClickListener() {
		...
		HorizontalPanel produktPanel = new HorizontalPanel();
		produktPanel.add(productsListWidget);
		produktPanel.add(pobierz);
		
		addingReportGrid.setWidget(0, 1, produktPanel);
		
		
		
		//addingReportGrid.setWidget(0, 2, pobierz);
		...
		final TextArea opis = new TextArea();
		opis.setCharacterWidth(40);
		opis.setVisibleLines(10);
		addingReportGrid.setWidget(7, 1, opis);
		...
		final Button dodaj = new Button("Dodaj zgloszenie", new ClickListener() {
			...
			public void onClick(Widget sender) {
				...
				zgloszenie.setDataZgloszenia(data);
				zgloszenie.setNazwiskoZglaszajacego(nazwisko.getText());
				zgloszenie.setImieZglaszajacego(imie.getText());
				zgloszenie.setTelefonZglaszajacego(telefon.getText());
				WagaProblemu wagaProblemu = new WagaProblemu();
				wagaProblemu.setNrWagi((short)Integer.parseInt(waga.getText()));
				zgloszenie.setWagaKlienta(wagaProblemu);
				zgloszenie.setOpisProblemu(opis.getText());
				zgloszenie.setProdukt(productsListWidget.getItemText(productsListWidget.getSelectedIndex()));
				zgloszenie.setSerwisant(userName);
				
				designerController.addNotification(dodaneZgloszenie = zgloszenie);
			}
		});
		
		addingReportGrid.setWidget(8, 1, dodaj);
